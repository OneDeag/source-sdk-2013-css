...
// Purpose: 
//-----------------------------------------------------------------------------
CHudBaseDeathNotice::CHudBaseDeathNotice( const char *pElementName ) :	CHudElement( pElementName ), BaseClass( NULL, "HudDeathNotice" )
{
...
void CHudBaseDeathNotice::FireGameEvent( IGameEvent *event )
{
	...
	if ( !m_DeathNotices[iMsg].iconDeath && m_DeathNotices[iMsg].szIcon )
	{
		...
		if ( !m_DeathNotices[iMsg].iconDeath )
			...
			m_DeathNotices[iMsg].iconDeath = GetIcon( "d_skull_cs", m_DeathNotices[iMsg].bLocalPlayerInvolved ? kDeathNoticeIcon_Inverted : kDeathNoticeIcon_Standard );
	}
}
